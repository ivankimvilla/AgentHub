...
<head>
    ...
    <style>
        ...
        * { box-sizing:border-box; } body { margin:0; color:var(--ink); background:#0c0f17; font-family:'DM Sans',sans-serif; line-height:1.7; } a { color:inherit; text-decoration:none; }
        .legal-nav,.legal-main,.legal-footer { width:min(820px,calc(100% - 40px)); margin:0 auto; } .legal-nav { padding:24px 0; border-bottom:1px solid var(--line); } .brand { display:flex; align-items:center; gap:14px; color:var(--ink); text-decoration:none; font-family:'Space Grotesk',sans-serif; font-size:24px; font-weight:700; } .mark { display:inline-grid; place-items:center; width:50px; height:50px; margin:0; color:#fff; background:#6658f5; border-radius:12px; } .mark svg { width:28px; height:28px; } .brand-accent { color:#8b72ff; } .legal-main { padding:54px 0 70px; } .eyebrow { color:var(--accent); font-size:12px; font-weight:700; letter-spacing:.08em; text-transform:uppercase; } h1,h2 { font-family:'Space Grotesk',sans-serif; letter-spacing:-.04em; line-height:1.1; } h1 { margin:14px 0 10px; font-size:clamp(38px,6vw,60px); } .updated { color:#7f899d; font-size:12px; } .legal-copy { margin-top:40px; border-top:1px solid var(--line); } h2 { margin:0; padding-top:28px; font-size:19px; } p,li { color:var(--muted); font-size:14px; text-align:justify; } .legal-footer { padding:24px 0 34px; border-top:1px solid var(--line); color:var(--muted); font-size:13px; } .legal-footer a { margin-left:18px; }
    </style>
</head>
<body>
    <nav class="legal-nav"><a class="brand" href="{{ route('home') }}" aria-label="AgentHub home"><span class="mark"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="3" y="11" width="18" height="10" rx="2"/><circle cx="12" cy="5" r="2"/><line x1="12" y1="7" x2="12" y2="11"/><path d="M8 15h.01M12 15h.01M16 15h.01" stroke-width="2.5"/></svg></span><span>Agent<span class="brand-accent">Hub</span></span></a></nav>
